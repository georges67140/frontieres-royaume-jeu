 ...
 try{
  for(const config of [
   ...
   try{
    ...
    assert.equal(await page.evaluate(()=>RDNDebug.view.scene.isReady()),true);
    ...
    await page.locator('#view').click();await page.waitForFunction(()=>RDNDebug.view.camera.radius<17,null,{timeout:30000});
    ...
    // Restart after victory must rebuild a usable WebGL scene, not lose the context.
    await page.evaluate(()=>{RDNDebug.game.state='running';RDNDebug.game.enemy.morale=0;RDNDebug.game.step(.02);});
    await page.locator('#next').click();
    await page.waitForFunction(()=>RDNDebug.ready&&RDNDebug.game.wave===2,null,{timeout:45000});
    await page.locator('[data-command="shield"]').click();
    assert.equal(await page.evaluate(()=>RDNDebug.game.player.order),'shield');
    assert.equal(await page.evaluate(()=>RDNDebug.view.scene.isReady()),true);
